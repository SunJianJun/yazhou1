 ...
 * @param {json} req - json形式：(senderID:“dfdf",type:"takeoff|shift",senderTitle:"123asd456",messageObj：{
...
var sendAbnormalMessage = function (req, res) {
  ...
  var senderID = req.body.senderID;
  var messType = req.body.type;
  var senderTitle = req.body.senderTitle;
  // var receiverInfo=req.body.receiverInfo;
  //如果没有类型，或者类型不是广播，就返回
  if (!req.body.type || (messType != "takeoff" && messType != "shift" ) || !messType || !senderTitle) {
    // //console.log("客户端发来的json有空值");
    res.send({error: "客户端发来的json有空值"});
    return;
  }
  ;
  console.log(senderID, messType, senderTitle)
  var recieverIds = [];
  ...
  persontieleDAO.getpersontitle(senderTitle, function (err, obj) {
    if (err) {
      res.send({error: null})
    } else {
      var parentTitle = obj.parentTitle;//取到上级title
      parentTitle=parentTitle?parentTitle:senderTitle;
      ...
      personDAO.gettitleToperson(parentTitle, function (personerr, personobj) {
        if (personerr) {
          ...
          for (var d = 0, receiverInfo = []; d < personobj.length; d++) {
            receiverInfo.push(personobj[d]._id)
          }
          if (receiverInfo && receiverInfo.length > 0) {
            //receiverInfo这是title id的数组
            ...
            // console.log('gettitleIdsToperson 查询所有title:' + receiverInfo + '发送的消息:' + personobj.length);

            var output = new Array();
            var abnormalID = uuid.v1();
            // console.log('messageObj ：' + '<>' + req.body.messageObj);
            // console.log(req.body.messageObj)
            var messageObj = req.body.messageObj ? req.body.messageObj : {};
            ...
            for (var index = 0; index < personobj.length; index++) {
              console.log(personobj[index]._id)
              messageDAO.sendBroadcast(messageObj, senderID, personobj[index]._id, function (err, obj) {
                if (!err) {
                  // //console.log('sendAMessage 查询所有'+senderID+'发送的消息:'+obj._id);
                  output.push(obj);
                } else {
