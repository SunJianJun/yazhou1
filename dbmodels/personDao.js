...
PersonDAO.prototype.saveandRegister = function(obj,departmentid, callback,rolestr) {
    ...
    if(!obj._id){
                        ...
                        if (!isExisted)
                        {
                            ...
                            departmentModel.update({_id:doc._id,persons:doc.persons},function(err,docs){//更新
                                ...
                                else
                                {
                                    ...
                                    if(!isdExisted)
                                    Personmodel.update({_id:instance._id,departments:instance.departments.push(departmentid)},function(err,docs){//更新
										if(err)
                                        {
        ...
        if(!isdExisted)
            Personmodel.update({_id:instance._id,role:'worker',departments:instance.departments.push(departmentid)},function(err,docs){//更新
                    if(err)
                    {
                        //console.log('人员添加部门update 失败：'+err);
                        callback(err,null);
...
PersonDAO.prototype.changePersonStatus = function(personid,status, callback,role) {
    ...
    Personmodel.findOne({_id: personid},{personlocations: 0}, function (err, doc) {
            if(!err){
                doc.status=status;//1,正常;0离职;2请假;3旷工
...
PersonDAO.prototype.getPersonLatestPositionInTimespan = function(personid,startTime,endTime,outcallback) {
    ...
    Personmodel.aggregate()
        ...
        .match({"personlocations.positioningdate":
            {
                 "$gte": new Date(startTime),
                "$lte":new Date(endTime)
            }
...
var addpersontotopdepartment=function (personobj,rolestr,callback) {
    departmentModel.findOne({parent:null},function(err, firstDepartment){
        ...
        if(!isExisted)      		personobj.departments.push({role:rolestr,
        ...
        personobj.save(function(err){
            ...
            if(!isExistedren)
                firstDepartment.persons.push({role:rolestr,
                    person:personobj
                });
            ...
            if(personObjs && personObjs.length>0){
                ...
                departmentModel.find({_id:{$in:dptids}},{persons:1}).exec(function (err,dprtsObjs) {
                    ...
                    if(dprtsObjs && dprtsObjs.length>0){
                        ...
                        Personmodel.find({_id:{$in:workmates},status:{$gt:0}},{personlocations: 0,images: 0}).exec(function (err,workmatesObjs) {
                            if(!err){
...
PersonDAO.prototype.getPersonsByDepartment = function(dptIds, callback) {
    // dptIds
    console.log('called Person findOne by mobile'+mobile);//mobile
    Personmodel.findOne({'mobile':mobile}, function(err, obj){
        callback(err, obj);
...
//ObjectId("58bff0836253fd4008b3d41b")
daoObj.getPersonLatestPositionInTimespan('58bff0836253fd4008b3d41b','2017-03-11','2017-03-13 24:00:00');
